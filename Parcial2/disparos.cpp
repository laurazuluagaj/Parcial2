        ...
        for(vel=10; vel<=400; vel++){
            for(; angle<90; angle++){
                ...
                for(t=0;t<=tiempo-2;t++){
                    ...
                    posDisparo[0] = velBala[0]*(t+2);
                    posDisparo[1] = posO[1] + velBala[1]*(t+2) - (0.5*g*(t+2)*(t+2));
                    if((sqrt(pow((posDisparo[0]-posBala[0]),2)+pow((posDisparo[1]-posBala[1]),2))<=radioD)&&(sqrt(pow((posD[0]-posBala[0]),2)+pow((posD[1]-posBala[1]),2))>radioD)&&(sqrt(pow((posO[0]-posBala[0]),2)+pow((posO[1]-posBala[1]),2))>radioD)){
                     impacto = 1;
                     break;
                    }
                    if(posBala[1]<0) break;
